/* ****************************************************
  Non-truncated regression model for one target variable.
   **************************************************** */
...
parameters {
  real<lower=0> phi_ss;    // within-event/within-site standard deviation
  real<lower=0> tau;       // between-event standard deviation
  real<lower=0> phi_s2s;   // site-to-site standard deviation
...
model {
  ...
  target += normal_lpdf(Y | mu_rec, phi_ss);
}
